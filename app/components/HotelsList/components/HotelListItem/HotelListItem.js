...
export default class HotelListItem extends React.Component { // eslint-disable-line react/prefer-stateless-function
  render() {
      ...
      <Container key={item.listingNumber}>
        ...
        <Carousel autoPlay >
                {item.images.map(image=>{ return(
                    <div>
                      <img src={image.c6_uri} key={image.c6_uri} />
                    </div>)})
                  }
          </Carousel>
          </Col>
          ...
          <Col >
           <span>  {item.averageRating && <StarRatingComponent
              name={`StarRatingComponent_${item.listingNumber}`}
              starCount={5}
              value={item.averageRating}
           /> }  {item.reviewCount ? `${item.reviewCount} reviews` : ''}</span>
          </Col>
          <Col>
          ...
          <Col>
          <span>  <span className="detailsProp">Bedrooms:  </span>{item.bedrooms}</span>
          </Col>
          {item.bathrooms && <Col>
          ...
          <Col>
          <span>  <span className="detailsProp">Pets:  </span>{item.petsAllowed ? 'Allowed': 'Not allowed'}</span>
          </Col>
